...
如果你下载的是 release 源码包，或者已经 clone 到本地，也可以直接安装仓库里的 Codex skill：
...
安装后可直接用 `synquest` 命令；如果你在仓库里本地运行，也可以继续用 `python3 functions/synquest/cli.py`。
...
- 独立 figure track：从带图知识源截图，绑定近缘文字，并生成要求解释图意的带图题
...
对于 `pdf/png/jpg/jpeg` 这类带图知识源，SynQuest 现在提供一条独立的 figure track：
...
- 语义检索生成示例：[`example/data/generated/synquest-semantic-thirty.json`](example/data/generated/synquest-semantic-thirty.json)
...
- 现有题库负责“系统已经整理了哪些题”
- 生成题负责“系统可以扩展出哪些新题”
- 正式题库中已并入 30 道 `SynQuest` 语义检索示例题

## Python 接口

可复用能力位于 [`functions/synquest/`](functions/synquest/)；安装后也可以直接通过 `synquest` 包导入。
...
- [`functions/synquest/figure_track.py`](functions/synquest/figure_track.py): 独立图示截图、近缘文字绑定与带图题生成
...
- adaptive similarity fallback: 当首轮近重复过滤过严时，允许补入高同源但不完全重复的新题
- rule-based prompt diversification: 对高相似题干做轻量改写，扩充本地无 LLM 场景下的题目数量
- `Poppler` 工具链：`pdftotext`、`pdfinfo`、`pdfimages`
- `pdftoppm`: 带图 PDF 页面截图渲染
- neighbor text context window: 用同页与相邻知识条目补全图意解释
- keyword-overlap distractor retrieval: 为带图题生成主题级干扰项
